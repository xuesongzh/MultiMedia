...
static void rtp_h264_unpack_destroy(void *p) {
    ...
    if (unpacker->ptr) free(unpacker->ptr);
...
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
*/
static int rtp_h264_unpack_stap(struct rtp_decode_h264_t *unpacker, const uint8_t *ptr, int bytes, uint32_t timestamp,
                                int stap_b) {
    int r, n;
    uint16_t len;
    uint16_t don;
...
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
*/
static int rtp_h264_unpack_mtap(struct rtp_decode_h264_t *unpacker, const uint8_t *ptr, int bytes, uint32_t timestamp,
                                int n) {
    int r;
    uint16_t dond;
    uint16_t donb;
...
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
*/
static int rtp_h264_unpack_fu(struct rtp_decode_h264_t *unpacker, const uint8_t *ptr, int bytes, uint32_t timestamp,
                              int fu_b) {
    int r, n;
    uint8_t fuheader;
    // uint16_t don;
    ...
    r = 0;
    n = fu_b ? 4 : 2;  // payload前面有几个字节是供fu_a或fu_b使用， fu_a只有2个字节; fu_b是4个字节
...
static int rtp_h264_unpack_input(void *p, const void *packet, int bytes) {
    ...
    unpacker = (struct rtp_decode_h264_t *)p;
    // 反序列化
    if (!unpacker || 0 != rtp_packet_deserialize(&pkt, packet, bytes) || pkt.payloadlen < 1) return -EINVAL;
    ...
    switch (nalt & 0x1F) {
        ...
        default:  // 1-23 NAL unit
            r = unpacker->handler.packet(unpacker->cbparam, (const uint8_t *)pkt.payload, pkt.payloadlen,
                                         pkt.rtp.timestamp, unpacker->flags);
            unpacker->flags = 0;
            unpacker->size = 0;
            return 0 == r ? 1 : r;  // packet handled
    }
