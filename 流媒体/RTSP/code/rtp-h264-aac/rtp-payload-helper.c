...
int rtp_payload_check(struct rtp_payload_helper_t* helper, const struct rtp_packet_t* pkt) {
    ...
    if (pkt->rtp.timestamp != helper->timestamp) {
        ...
        // 1 - packet lost on timestamp changed, can't known losted packet is at old packet tail or new packet start, so
        // two packets mark as packet lost
        if (0 != lost) helper->lost = lost;
    }
...
int rtp_payload_write(struct rtp_payload_helper_t* helper, const struct rtp_packet_t* pkt) {
    ...
    if (size > helper->maxsize || size < 0) return -EINVAL;
...
int rtp_payload_onframe(struct rtp_payload_helper_t* helper) {
    ...
    if (helper->size > 0
        ...
        r = helper->handler.packet(helper->cbparam, helper->ptr, helper->size, helper->timestamp,
    ...
    if (helper->lost) helper->__flags |= RTP_PAYLOAD_FLAG_PACKET_LOST;
